...
use crate::{repositories::event::{event_repo::EventRepository, models::{EventFilter, NewEvent, EventData}}, errors::parse_error, templates::event::{EventLiteTemplate, EventsTemplate, EventTemplate}};

#[get("/event")]
pub async fn get_events(params: web::Query<EventFilter>, event_repo: web::Data<EventRepository>) -> HttpResponse {
    ...
    if let Ok(events) = result {
        let lite_events = events.into_iter().map(|event| EventLiteTemplate {
            id: event.id,
        ...
        return HttpResponse::Ok().content_type("text/html").body(body.expect("Should be okay now."));
    }
...
#[get("/event/{event_id}")]
pub async fn get_event(event_id: web::Path<String>, event_repo: web::Data<EventRepository>) -> HttpResponse {
    ...
    if let Ok(event) = result {
        let template = EventTemplate {
            ...
            avatar_url: event.avatar_url,
            name: event.name,
            description: event.description.unwrap_or("No description set.".to_string()),
        ...
        return HttpResponse::Ok().content_type("text/html").body(body.expect("Should be valid."))
    }
...
#[post("/event")]
pub async fn create_event(new_event: web::Form<NewEvent>, event_repo: web::Data<EventRepository>) -> HttpResponse {
    ...
    if let Ok(event) = result {
        ...
        return HttpResponse::Created().content_type("text/html").body(body.expect("Should be valid."));
    }
...
pub async fn delete_event(event_id: web::Path<String>, event_repo: web::Data<EventRepository>) -> HttpResponse {
    let id_parse = Uuid::from_str(event_id.into_inner().as_str());
    if id_parse.is_err() {
        return HttpResponse::BadRequest().body(parse_error(http::StatusCode::BAD_REQUEST));
    }
